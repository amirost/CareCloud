    ...
    return {
        ...
        applyOptimalSolution() {
            ...
            gameState.optimalPathSolution.forEach(solutionPath => {
                ...
                // 1. Marquer les utilisateurs et les paires comme connectés
                ...
                // 2. Créer les liens virtuels (utilisateur <-> antenne)
                ...
                // 3. Parcourir le chemin physique (entre les antennes)
                for (let i = 1; i < path.length - 2; i++) {
                    const sourceNodeId = path[i];
                    const targetNodeId = path[i + 1];

                    const sourceNode = gameState.cy.getElementById(sourceNodeId);
                    const targetNode = gameState.cy.getElementById(targetNodeId);

                    if (sourceNode && targetNode) {
                        // Trouver le lien physique entre ces deux nœuds
                        const edge = sourceNode.edgesWith(targetNode).filter(e => !e.data('virtual')).first();
                        
                        if (edge.length > 0) {
                            const edgeId = edge.id();
                            
                            // Ajouter la couleur à la liste des liens utilisés
                            if (!gameState.usedLinks.has(edgeId)) {
                                gameState.usedLinks.set(edgeId, []);
                            }
                            gameState.usedLinks.get(edgeId).push(color);
                            
                            // DESSINER LE LIEN EN UTILISANT LA FONCTION GLOBALE
                            if (gameState.eventHandlers && gameState.eventHandlers.updateEdgeVisualization) {
                                gameState.eventHandlers.updateEdgeVisualization(edge);
                            } else {
                                console.error("updateEdgeVisualization function not found on gameState.eventHandlers");
                            }
                        } else {
                            console.error(`Physical edge not found between ${sourceNodeId} and ${targetNodeId}`);
                        }
                    }
                }
            });

            // 4. Désactiver les liens non utilisés
            this.optimizeUnusedLinks();
